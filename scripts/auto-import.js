...
async function refreshCaches() {
  ...
  try {
    // Try the combined refresh function
    const { data, error } = await supabase.rpc('refresh_all_sms_caches');

    if (error) {
      console.log('   Using individual refresh...');
      ...
      console.log('   ✅ Revenue cache refreshed');
    } else {
      console.log('   ✅ All caches refreshed!');
    }
  } catch (err) {
    console.error('   ⚠️  Cache refresh note:', err.message);
    console.log('   Run manually: SELECT refresh_all_sms_caches();');
  }
}
